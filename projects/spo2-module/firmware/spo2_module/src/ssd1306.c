...
const uint8_t hrt_rt_logo[] = {
  
};

const uint8_t ASCII[][ASCII_PIXEL_WIDTH] = {
...
//  ,{0x1f, 0x24, 0x7c, 0x24, 0x1f} // 7f UT sign
	,{0x3e, 0x20, 0x7c, 0x54, 0x7c}  // LB for Long Beach
};

static uint8_t screen_buffer[SCREEN_SIZE];
...
void SSD1306_Init(){
    ...
    SSD1306_Command(DISPLAY_START_LINE);

    SSD1306_Command(SET_DISPLAY_OFFSET);
    SSD1306_Command(NO_OFFSET);

    SSD1306_Command(SEGMENT_REMAPPING);
    SSD1306_Command(COM_OUTPUT_SCAN_DIRECTION);

    SSD1306_Command(SET_COM_PINS);
    SSD1306_Command(COM_PINS); // refer to macro
    
    SSD1306_Command(SET_MEMORY_ADDRESSING_MODE);
    SSD1306_Command(PAGE_ADDRESSING_MODE);
...
void SSD1306_SetCursor(uint8_t x, uint8_t y){
    ...
    SSD1306_Command(PAGE_ADDRESS_START + y);
    SSD1306_Command(SET_LOWER_COLUMN + (x & 0x0F)); // keeping only the lower 4 bits of x
    SSD1306_Command(SET_UPPER_COLUMN + ((x >> 4) & 0x0F)); // move the command down, keep the lower 4 bits of x
}

void SSD1306_Clear(void){
    // Clear the screen by writing zeroes to the entire screen and reset the cursor
    for (uint32_t i = 0; i < (SCREEN_HEIGHT / 8); i++){
...
void SSD1306_DrawChar(char c){
    int i;
    SSD1306_Data(0x00);
    for(i = 0; i < ASCII_PIXEL_WIDTH; i++){
        SSD1306_Data(ASCII[c - 0x20][i]);
    }
    SSD1306_Data(0x00);
}
...
void SSD1306_DrawBitmap(const uint8_t *bitmap){
    int i;
    SSD1306_Command(SET_COLUMN_ADDRESS);
    SSD1306_Command(0); // start column
    SSD1306_Command(SCREEN_WIDTH - 1); // end column

    SSD1306_Command(SET_PAGE_ADDRESS);
    SSD1306_Command(0); // start page
    SSD1306_Command((SCREEN_HEIGHT / 8) - 1); // end page
